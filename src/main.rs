...
async fn main() {
    ...
    let prompt = Message::new(Role::User, "What time is it now?".to_string());
    agent.add_message(prompt);

    println!("=== Tool Integration Demo ===");
    println!("Calling model with message: {}", &agent.messages[0].content);
    println!();

    println!("Step 1: Calling model...");
    let response = match agent.call().await {
        Ok(res) => res,
        Err(e) => {
            println!("Error: {}", e);
            return;
        ...
        if !choices.is_empty() {
            ...
            if let Some(calls) = tool_calls {
                if !calls.is_empty() {
                    ...
                    let final_response = agent.call().await.unwrap_or(json!({}));
